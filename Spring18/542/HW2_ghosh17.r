...
{
  if(b < 0 & abs(b) > lambda/2)
    b = b + lambda/2
  else if(b > 0 & abs(b) > lambda/2)
    b = b - lambda/2
  else
    b = 0
  b
...
plot(res$lambda[50:100], res$rmse[50:100], xlab="lambda", ylab="Test RMSE", type = "b", main = "Test RMSE vs Lambda for lowest 50 values of lambda", pch = 20)
